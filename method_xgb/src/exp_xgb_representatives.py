"""用「每個相關組一個代表」的欄位訓練 XGBoost,量每一欄的重要性;並和 242 欄全放的模型比較。

  代表: experiments/feature_representatives_g90.csv(|ρ| ≥ 0.9 分群、162 欄,選法不看報酬)。
...
輸出 experiments/xgb_representatives_g90.csv(每欄重要性)、xgb_representatives_g90_summary.json(兩組模型的成績)
"""
...
cat = pd.read_csv(os.path.join(R, "method_xgb", "config", "feature_catalog_top100.csv")); ALL = cat.name.tolist()
REPS = pd.read_csv(os.path.join(P.XGB_EXP, "feature_representatives_g90.csv")).representative.tolist()
...
def run(name, feats, importance):
    ...
    return summ, imp, gain, ic


T0 = time.time()
s_rep, imp, gain, ic_rep = run("代表 162 欄", REPS, importance=True)
s_all, _, _, ic_all = run("全部 242 欄", ALL, importance=False)
d = (ic_rep - ic_all).dropna()
out = {"representatives": s_rep, "all242": s_all, "diff_rep_minus_all": {"ic_mean": float(d.mean()), "t": block_t(d.values), "corr_daily_ic": float(np.corrcoef(ic_rep.loc[d.index], ic_all.loc[d.index])[0, 1])},
       "elapsed_s": round(time.time() - T0)}
json.dump(out, open(os.path.join(P.XGB_EXP, "xgb_representatives_g90_summary.json"), "w"), ensure_ascii=False, indent=1)
fam = dict(zip(cat.name, cat.family)); rows = []
for f in REPS:
    v = pd.DataFrame(imp[f], columns=["year", "drop"]); by = v.groupby("year")["drop"].mean() if len(v) else pd.Series(dtype=float)
    rows.append({"feature": f, "family": fam[f], "perm_mean": by.mean() if len(by) else np.nan, "perm_pos_folds": int((by > 0).sum()), "n_folds": int(len(by)),
                 "perm_t": (by.mean() / (by.std(ddof=1) / np.sqrt(len(by)))) if len(by) > 1 and by.std(ddof=1) > 0 else np.nan,
                 "gain_share": float(np.mean(gain[f])) if gain[f] else 0.0, **{f"perm_{int(k)}": float(x) for k, x in by.items()}})
pd.DataFrame(rows).sort_values("perm_mean", ascending=False).to_csv(os.path.join(P.XGB_EXP, "xgb_representatives_g90.csv"), index=False)
print(json.dumps(out, ensure_ascii=False, indent=1)); print("[DONE]", round(time.time() - T0), "s", flush=True)
