"""209 欄(157 個舊代表 + 52 個新特徵:籌碼對齊當日、隔夜 / 日內、長週期動能)訓練 XGBoost,量每一欄的重要性;並和 162 欄舊代表配對比較。
...
輸出 experiments/xgb_v2_209.csv(每欄重要性)、xgb_v2_209_summary.json(兩組模型的成績與配對差)
"""
...
T0 = time.time()
s_rep, imp, gain, ic_rep, top_rep = run("新 209 欄", REPS, importance=True)
s_all, _, _, ic_all, top_all = run("舊 162 欄", OLD162, importance=False)
d = (ic_rep - ic_all).dropna()
dt = (top_rep - top_all).dropna()
out = {"v2_209": s_rep, "old_162": s_all, "diff_209_minus_162": {"ic_mean": float(d.mean()), "ic_t": block_t(d.values), "corr_daily_ic": float(np.corrcoef(ic_rep.loc[d.index], ic_all.loc[d.index])[0, 1]),
                                                                      "top10_bp": float(dt.mean()), "top10_t": block_t(dt.values)},
       "elapsed_s": round(time.time() - T0)}
json.dump(out, open(os.path.join(P.XGB_EXP, "xgb_v2_209_summary.json"), "w"), ensure_ascii=False, indent=1)
...
pd.DataFrame(rows).sort_values("perm_mean", ascending=False).to_csv(os.path.join(P.XGB_EXP, "xgb_v2_209.csv"), index=False)
