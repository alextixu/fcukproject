...
class ChartGCNDataset(Dataset):
    def __init__(
        ...
        for ticker, df in stock_data.items():
            ...
            for end in range(window, len(df) - horizon, stride):
                start = end - window
                # 決策日 = window 最後一天 (end-1);
                # label = horizon 個交易日後相對決策日的漲跌 (horizon=1 即論文隔日標籤)
                decision_date = df.index[end - 1]
                if min_date is not None and decision_date <= min_date:
                    continue
                label = 1 if close[end - 1 + horizon] > close[end - 1] else 0
