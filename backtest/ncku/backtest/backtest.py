...
class Stock_Information_Memory:
    ...
    def get_stock_information(cls, stock_code: str, date: str):
        ...
        try:
            ...
            # 原版只在 MultiIndex 有重複列(loc 回傳 DataFrame)時能跑;索引唯一時 loc 回傳 Series,
            # get_price() 會 TypeError。這裡統一成 {欄位: {(code, date): 值}} 的形狀,其餘邏輯不變。
            if isinstance(row, pd.Series):
                return {k: {(stock_code, date): v} for k, v in row.to_dict().items()}
            return row.to_dict()
        except KeyError:
            return {}
